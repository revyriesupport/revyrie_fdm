...
class CartItems extends HTMLElement {
  ...
  onChange(event) {
    ...
    } else if(event.target.className == "variant__input") {
      var variant_id = event.target.value;
      var quantity = parseInt(event.target.dataset.cartQuantity);
      window.theme.addToCart({
        id: variant_id,
        quantity: quantity
      }).then(() => {
          this.updateQuantity(parseInt(event.target.dataset.index)+1, 0);
        });
    }
  }
