...
export default {
  ...
  setup(props) {
    ...
    const variantChange = async (event) => {
      ...
      loading.value = true;
      var qty = document.getElementById(event.target.getAttribute("identifier")).querySelector('.variant_quantity').value;
      let newVariantData = generateBodyObject({
          id: variantId,
          quantity: qty,
          properties: {},
          selling_plan: null
        });
        await generateFetchRequest("/cart/add.js", "POST", newVariantData, null);
        requestUpdate(0);
    };
          ...
          <div class="col-span-2">
            <div>
              ...
              <div class="variantlistbox">
                ...
                <select @change="variantChange" class="variant__input" :identifier="selectedVariantId" v-model="selectedVariantId">
                  <option  v-for="variant in line.variant_data"   :value="variant.id" :data-aviliable="variant.available" >{{ variant.title }}</option> 
                  
                </select>
              </div>
              <div class="" v-for="tag in line.tag"><div class="cart-side-final-sale" v-if="tag.includes('badge:final sale')">Final sale</div><div v-else=""></div></div>
            
              <div class="cart_slide_preorder" v-for="tag in line.tag">
                <div class="cart-side-final-sale" v-if="tag.includes('preorder')">
                Preorder, will ship <span v-if="line.product_description.includes('<!-- back-order -->')"><b>{{ line.product_description.split('<!-- back-order -->') }}aa</b></span><span class="" v-else="">when it becomes available</span>
                </div>
                <div v-else=""></div>
              </div>
            ...
            <div class="flex flex-col items-start">
              <div
                class="flex items-center space-x-2 border qty-handle-button" :id="selectedVariantId"
