  ...
  getters: {
    ...
    listBundleItems() {
        ...
        this.items.forEach((item) => {
          if(typeof item.properties['_product_from'] != 'undefined') {
    ...
    finalTotal() {
      return calculateTotal(this.items, (item) => item.final_line_price) || 0;
    }
  },
  actions: {
    open() {
    ...
    async fetchCart() {
      ...
      try {
        ...
        await this.getCartItemsTagData(this.items);
      } catch (error) {
        this.isLoading = false
    ...
    async getCartItemsTagData(items) { 
      items.forEach((item) => {
        item.tag = [];
        fetch('/products/' + item.handle + '.js')
        .then(response => response.json())
        .then(product_data =>
          item.tag = product_data.tags)
        .catch(err => { throw err });
      });
      return items;
    },
          
    async getCartItemTagData(item) { 
      item.tag = [];
      fetch('/products/' + item.handle + '.js')
      .then(response => response.json())
      .then(product_data =>
    ...
    async removeBundle(requestBody){
      ...
      try {
          ...
          if(cartTemplate){
            const cart_item_section = document.getElementById('main-cart-items').dataset.id;
            const main_cart_footer_header = document.getElementById('main-cart-footer-header').dataset.id;
            const main_cart_footer = document.getElementById('main-cart-footer').dataset.id;
            const sections = cart_item_section + "," + main_cart_footer_header + "," + main_cart_footer;
            ...
            .then((responseText) => {
              ...
              const items_html = new DOMParser().parseFromString(responseText[`${cart_item_section}`], 'text/html');
              const cartItems = items_html.querySelector('cart-items');
              document.getElementById("cart_items").innerHTML = cartItems.innerHTML;

              const cart_header_html = new DOMParser().parseFromString(responseText[`${main_cart_footer_header}`], 'text/html');
              const cartHeader = cart_header_html.querySelector('#main-cart-footer-header');
              document.getElementById("main-cart-footer-header").innerHTML = cartHeader.innerHTML;

              const cart_footer_html = new DOMParser().parseFromString(responseText[`${main_cart_footer}`], 'text/html');
              const cartFooter = cart_footer_html.querySelector('#main-cart-footer .js-contents');
              document.getElementById("main-cart-footer").innerHTML = cartFooter.innerHTML;

              if(this.items.length == 0)
              {
                document.getElementById("cart_items").classList.add("is-empty");
    ...
    async addToCart({ id, quantity = 1, properties = {}, selling_plan = null }) {
      ...
      try {
        const response =
          ...
          await this.getCartItemTagData(response.data);
